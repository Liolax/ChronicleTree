...
module ImageGeneration
  class TreeSnippetGenerator < BaseGenerator
    ...
    def draw_enhanced_tree_svg
      ...
      # Account for header (80px) and footer (70px), with larger safety margins for 4+ generations
      footer_margin = @tree_data.keys.length > 3 ? 100 : 70  # More space for 4+ generations
      available_height = CANVAS_HEIGHT - 80 - footer_margin - 60  # Extra margin for safety
      center_y = 80 + 30 + (available_height / 2)  # Start a bit lower
    ...
    def calculate_generation_spacing
      ...
      when 3
        90   # Slightly reduced for 3 generations
      when 4
        70   # More compact for 4 generations
      when 5
        60   # Very compact for 5 generations
      else
        # For 6+ generations, calculate to fit available space
        footer_margin = generations_count > 3 ? 100 : 70
        available_height = CANVAS_HEIGHT - 80 - footer_margin - 60  # Account for header, footer, margins
        spacing = available_height / generations_count
        [spacing, 50].max.to_i  # Minimum 50px spacing
      end
    end
