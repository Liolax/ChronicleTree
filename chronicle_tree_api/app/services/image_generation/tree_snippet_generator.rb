  ...
  class TreeSnippetGenerator < BaseGenerator
    ...
    def generate(root_person, options = {})
      ...
      begin
        ...
        create_tree_snippet
        filename = "tree_#{root_person.id}_#{SecureRandom.hex(8)}.jpg"
        ...
        ShareImage.create!(
          ...
          metadata: { generations: generations, people_count: total_people_count }
        )
    ...
    def svg_content
      <<~CONTENT
        <!-- Tree Title -->
        <text x="#{CANVAS_WIDTH/2}" y="100" text-anchor="middle" font-family="Arial, sans-serif" font-size="28" font-weight="bold" fill="#{COLORS[:text_light]}">
          #{escape_xml(@root_person.full_name)}'s Family Tree
        </text>
        <text x="#{CANVAS_WIDTH/2}" y="130" text-anchor="middle" font-family="Arial, sans-serif" font-size="16" fill="#{COLORS[:text_light]}">
          #{@generations} generations • #{total_people_count} family members
        </text>
        ...
        <!-- Footer -->
        <text x="80" y="#{CANVAS_HEIGHT - 40}" font-family="Arial, sans-serif" font-size="14" fill="#{COLORS[:text_light]}">
          Generated by ChronicleTree
        </text>
        <text x="#{CANVAS_WIDTH - 180}" y="#{CANVAS_HEIGHT - 40}" font-family="Arial, sans-serif" font-size="14" fill="#{COLORS[:text_light]}">
    ...
    def add_tree_header(image)
      ...
      subtitle = "#{@generations} generations • #{total_people_count} family members • #{relationship_count} connections"
    ...
    def add_family_stats_panel(image)
      ...
      # Add panel title
      stats_title = create_colored_text('📊 Family Statistics', 'sans bold 16', [59, 130, 246])
      ...
      # Total people
      total_text = create_colored_text("👥 #{total_people_count} family members", 'sans 14', [75, 85, 99])
      image = image.composite total_text, 'over', x: panel_x + 10, y: stats_y
      stats_y += line_spacing
      
      # Generations
      gen_text = create_colored_text("🔄 #{@generations} generations", 'sans 14', [75, 85, 99])
      image = image.composite gen_text, 'over', x: panel_x + 10, y: stats_y
      stats_y += line_spacing
      
      # Living members
      living_count = count_living_members
      living_text = create_colored_text("💚 #{living_count} living", 'sans 14', [34, 197, 94])
      ...
      if age_range
        age_text = create_colored_text("📅 Ages #{age_range}", 'sans 14', [75, 85, 99])
    ...
    def add_tree_footer(image)
      ...
      footer_text = "🌳 Generated by ChronicleTree • #{Date.current.strftime('%B %Y')} • Family Tree Visualization"
    ...
    def build_tree_structure
      ...
      # Build generations around the root person
      (-@generations..@generations).each do |gen_offset|
        structure[gen_offset] = find_people_in_generation(gen_offset)
      end
      
      structure
    end
    
    def find_people_in_generation(generation_offset)
    ...
    def add_title
      ...
      subtitle = "#{@generations} generations • #{total_people_count} family members"
