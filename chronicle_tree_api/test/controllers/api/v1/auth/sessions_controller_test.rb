...
class Api::V1::Auth::SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get token on sign in with valid credentials" do
    post api_v1_auth_sign_in_url, params: { user: { email: @user.email, password: 'Password123!' } }, as: :json
    assert_response :ok
  ...
  test "should not get token on sign in with invalid password" do
    post api_v1_auth_sign_in_url, params: { user: { email: @user.email, password: 'wrong_password' } }, as: :json
    assert_response :unauthorized
  end

  test "should sign out" do
    auth_headers = { 'Authorization' => sign_in_as(@user) }
    delete api_v1_auth_sign_out_url, headers: auth_headers, as: :json
